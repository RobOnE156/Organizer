...
// The hamburger button + slide-in navigation drawer, shared by the timeline top
// bar and the 3D showcase bar. `triggerClassName` lets each surface style the
// button (e.g. light-on-dark in the showcase).
export default function NavDrawer({
  ...
  return (
    <>
      <button className={triggerClassName} onClick={() => setOpen(true)} aria-label={t("nav.menu")} aria-expanded={open}>
        ☰
      </button>
      ...
      {open ? (
        <div className="drawer-wrap" onClick={() => setOpen(false)}>
          <nav className="drawer" onClick={(e) => e.stopPropagation()} aria-label={t("nav.menu")}>
            ...
            <a className="drawerlink" href="/showcase">
              <span className="di">✨</span> {t("nav.showcase")}
            </a>
