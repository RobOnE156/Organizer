...
// photos served through an outward share link. Returns the input unchanged if
// it isn't a well-formed JPEG.
export function stripJpegExif(buf: Uint8Array): Uint8Array {
  if (buf.length < 4 || buf[0] !== 0xff || buf[1] !== 0xd8) return buf; // not a JPEG (FFD8 = SOI)

  const parts: Uint8Array[] = [buf.subarray(0, 2)];
  let i = 2;
  while (i + 4 <= buf.length) {
    if (buf[i] !== 0xff) return buf; // unexpected structure — don't risk corrupting it
    ...
    if (len < 2 || segEnd > buf.length) return buf; // malformed — leave as-is
