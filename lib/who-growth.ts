// WHO Child Growth Standards (LMS method), 0–60 completed months.
// Source: WHO Multicentre Growth Reference Study (2006), via the open
// `zscorer` dataset. weight-for-age (kg) and length/height-for-age (cm), by
// sex. Each row is [ageMonths, L, M, S]. Verified against the published WHO
// medians (e.g. boy median at birth: 3.35 kg / 49.9 cm).

export type LmsRow = [month: number, l: number, m: number, s: number];
export type WhoMetric = "weight" | "height";
export type WhoSex = "male" | "female";

export const WHO_LMS: Record<WhoMetric, Record<WhoSex, LmsRow[]>> = {
  weight: {
    male: [
...
// z-scores of the standard percentile lines we draw as background bands.
...
// LMS → measurement value at a given z-score.
function lmsValue(l: number, m: number, s: number, z: number): number {
  return Math.abs(l) < 1e-7 ? m * Math.exp(s * z) : m * Math.pow(1 + l * s * z, 1 / l);
}

export type WhoCurve = { p: number; points: { month: number; value: number }[] };

// Percentile curves (P3…P97) for a metric+sex across 0–60 months.
export function whoCurves(metric: WhoMetric, sex: WhoSex): WhoCurve[] {
  const rows = WHO_LMS[metric][sex];
  return PERCENTILES.map(({ p, z }) => ({
    p,
    points: rows.map(([month, l, m, s]) => ({ month, value: lmsValue(l, m, s, z) })),
  }));
}

// z-score of a single measurement (linear-interpolating L,M,S between months),
// used to label a plotted point with its percentile.
export function zScoreFor(metric: WhoMetric, sex: WhoSex, month: number, value: number): number | null {
  const rows = WHO_LMS[metric][sex];
  if (month < 0 || month > 60 || value <= 0) return null;
  const lo = Math.floor(month);
  const hi = Math.min(60, Math.ceil(month));
  const a = rows[Math.min(lo, 60)];
  const b = rows[hi];
  if (!a || !b) return null;
  const f = hi === lo ? 0 : (month - lo) / (hi - lo);
  const l = a[1] + (b[1] - a[1]) * f;
  const m = a[2] + (b[2] - a[2]) * f;
  const s = a[3] + (b[3] - a[3]) * f;
  const z = Math.abs(l) < 1e-7 ? Math.log(value / m) / s : (Math.pow(value / m, l) - 1) / (l * s);
  return Number.isFinite(z) ? z : null;
}

// z-score → percentile (0–100), via the Abramowitz-Stegun normal-CDF approx.
